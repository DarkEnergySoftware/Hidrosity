...
public class LifeCounter {
	...
	public LifeCounter(Player player) {
		...
		fontGenerator = new FreeTypeFontGenerator(
				Gdx.files.internal("res/ui/lifesFont.ttf"));
		font = fontGenerator.generateFont(14);
	}

	public void render(SpriteBatch spriteBatch) {
		spriteBatch.draw(texture, 10,
				Gdx.graphics.getHeight() - 138 - texture.getHeight() * 2,
				texture.getWidth() * 2, texture.getHeight() * 2);
		font.setColor(Color.GRAY);
		font.draw(spriteBatch, String.valueOf(player.getLives()), 80,
				Gdx.graphics.getHeight() - 160);
		font.setColor(Color.WHITE);
		font.draw(spriteBatch, String.valueOf(player.getLives()), 78,
				Gdx.graphics.getHeight() - 158);
	}
